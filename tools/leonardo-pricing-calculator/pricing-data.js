// Leonardo API Pricing Calculator snapshot, captured 2026-08-01 (USD/generation).
// Aspect ratios share the same price within a selected quality/size tier.
const PRICING={image:{},video:{}};
const A=['2:3','1:1','16:9'], Z=[null];
function img(n,q,s,r,c){PRICING.image[n]={qualities:q,sizes:s,ratios:r,configs:q.flatMap((x,qi)=>s.map((y,si)=>({quality:x,size:y,cost:c[qi*s.length+si]})))}}
...
['Ideogram 4.0','Ideogram 3.0'].forEach(n=>img(n,['Low','Medium','High'],Z,[],[.0374,.0748,.1121]));
...
['Phoenix 1.0','Phoenix 0.9'].forEach(n=>img(n,['Fast','Quality','Ultra'],['Small 896×896','Medium 1024×1024','Large 1120×1120'],A,[.015,.015,.015,.0224,.0224,.0224,.0538,.0658,.0748]));
...
['Concept Art','Graphic Design','Illustrative Albedo','Leonardo Lightning','Lifelike Vision','Portrait Perfect','Stock Photography'].forEach(n=>img(n,['Fast','Quality'],['Small 888×888','Medium 960×960','Large 1024×1024'],A,[.0045,.0045,.006,.0239,.0239,.0239]));
function vid(n,d,res,rat,c){PRICING.video[n]={durations:d,resolutions:res,ratios:rat,configs:d.flatMap((x,di)=>res.map((y,ri)=>({duration:x,resolution:y,cost:c[di*res.length+ri]})))}}
function slider(n,min,max,res,rat,endpoints){PRICING.video[n]={durations:Array.from({length:max-min+1},(_,i)=>min+i),resolutions:res,ratios:rat,slider:true,configs:res.map((x,i)=>({resolution:x,min,max,minCost:endpoints[i][0],maxCost:endpoints[i][1]}))}}
