...
func TestGatewayHandlerSubmitUsageRecordTask_DroppedTaskSyncFallback(t *testing.T) {
	...
	var called atomic.Bool
	h.submitUsageRecordTask(context.Background(), func(context.Context) { called.Store(true) })
	close(release)
	require.True(t, called.Load(), "dropped usage task must run synchronously")
}
...
func TestOpenAIGatewayHandlerSubmitUsageRecordTask_DroppedTaskSyncFallback(t *testing.T) {
	...
	h.submitUsageRecordTask(context.Background(), func(context.Context) { called.Store(true) })
	close(release)
	require.True(t, called.Load(), "dropped OpenAI usage task must run synchronously")
}
