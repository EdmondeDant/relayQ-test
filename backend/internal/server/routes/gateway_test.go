...
func TestGatewayRoutesXAIImagesPathsAreRegistered(t *testing.T) {
	...
	RegisterGatewayRoutes(
		...
		servermiddleware.APIKeyAuthMiddleware(func(c *gin.Context) {
			...
			c.Set(string(servermiddleware.ContextKeyAPIKey), &service.APIKey{
				GroupID: &groupID,
				Group:   &service.Group{Platform: service.PlatformXAI},
			})
			c.Next()
		}),
		nil,
...
func TestGatewayRoutesXAIVideosPathsAreRegistered(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router := gin.New()
	...
	for path, body := range postCases {
		req := httptest.NewRequest(http.MethodPost, path, strings.NewReader(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()

		router.ServeHTTP(w, req)
		require.NotEqual(t, http.StatusNotFound, w.Code, "path=%s should hit xai videos handler", path)
	}
