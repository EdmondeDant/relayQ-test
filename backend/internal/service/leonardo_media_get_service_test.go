...
func TestLeonardoMediaGetServiceReturnsOwnedVideo(t *testing.T) {
	...
	require.Equal(t, "https://cdn.example/video.mp4", result.VideoURL)
	require.Equal(t, "https://cdn.example/video.mp4", result.URL)
	require.Equal(t, "https://cdn.example/video.mp4", result.ResultURL)
	require.Equal(t, &LeonardoMediaGetURLBundle{VideoURL: "https://cdn.example/video.mp4", URL: "https://cdn.example/video.mp4"}, result.Content)
...
func TestLeonardoMediaGetServiceReconcilesTerminalBillingWithoutNetwork(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, openAICompatibleMediaStatus(test.status), result.Status)
...
func TestLeonardoMediaGetServiceDuePollUsesOneGetAndNeverPosts(t *testing.T) {
	...
	result, err := leonardoMediaGetService(repository, accounts, upstream).Get(context.Background(), leonardoMediaGetInput())
	require.NoError(t, err)
	require.Equal(t, openAICompatibleMediaStatus(GenerationJobStatusQueued), result.Status)
...
func TestLeonardoMediaGetServiceConcurrentCASOneSuccessOneConflict(t *testing.T) {
	...
	for _, outcome := range []getOutcome{first, second} {
		...
		require.Equal(t, openAICompatibleMediaStatus(GenerationJobStatusQueued), outcome.result.Status)
	}
