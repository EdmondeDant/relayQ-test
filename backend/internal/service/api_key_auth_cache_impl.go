...
const apiKeyAuthSnapshotVersion = 12 // v12: reload snapshots for managed API key metadata
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		Name:           apiKey.Name,
		ClientApp:      apiKey.ClientApp,
		Managed:        apiKey.Managed,
		ManagedPurpose: apiKey.ManagedPurpose,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		Key:            key,
		Name:           snapshot.Name,
		ClientApp:      snapshot.ClientApp,
		Managed:        snapshot.Managed,
		ManagedPurpose: snapshot.ManagedPurpose,
