...
func (p playgroundJobPayload) buildVideoRequest(model string) (string, json.RawMessage, error) {
	...
	if strings.EqualFold(strings.TrimSpace(platform), PlatformLeonardo) {
		...
		if (model == "kling-2.1" || model == "kling-2.5-turbo-standard") && (p.Media.InputReference == nil || strings.TrimSpace(p.Media.InputReference.URL) == "") {
			return "", nil, fmt.Errorf("%w: first-frame image is required by this Leonardo video model", ErrPlaygroundInvalidInput)
		}
		if p.Media.InputReference != nil && strings.TrimSpace(p.Media.InputReference.URL) != "" {
			if !SupportsLeonardoVideoStartFrame(model) {
