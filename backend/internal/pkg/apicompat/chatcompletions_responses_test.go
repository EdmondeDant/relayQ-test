...
func TestChatCompletionsToResponses_ImageURLStringVariant(t *testing.T) {
	content := `[{"type":"text","text":"Describe this"},{"type":"image_url","image_url":"data:image/png;base64,abc123"}]`
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(content)},
...
func TestChatCompletionsToResponses_InputImageVariant(t *testing.T) {
	content := `[{"type":"input_text","text":"Describe this"},{"type":"input_image","image_url":"data:image/png;base64,abc123"}]`
	req := &ChatCompletionsRequest{
		Model: "gpt-4o",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(content)},
...
func TestChatCompletionsToResponses_VideoURLRejectedByResponsesBridge(t *testing.T) {
	content := `[{"type":"text","text":"Describe this video"},{"type":"video_url","video_url":{"url":"https://example.com/video.mp4"}}]`
	req := &ChatCompletionsRequest{
		Model: "gpt-5.5",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(content)},
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "video content")
}

func TestChatCompletionsToResponses_AudioURLRejectedByResponsesBridge(t *testing.T) {
	content := `[{"type":"text","text":"Describe this audio"},{"type":"audio_url","audio_url":"https://example.com/audio.wav"}]`
	req := &ChatCompletionsRequest{
		Model: "gpt-5.5",
		Messages: []ChatMessage{
			{Role: "user", Content: json.RawMessage(content)},
	...
	_, err := ChatCompletionsToResponses(req)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "audio content")
}
