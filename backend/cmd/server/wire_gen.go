...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	mediaProductRepository := repository.NewMediaProductRepository(db)
	mediaCatalogService := service.NewMediaCatalogService(mediaProductRepository)
	generationJobRepository := repository.NewGenerationJobRepository(client, db)
	canvasResourceRouteRepository := repository.NewCanvasResourceRouteRepository(db)
	canvasRoutingService := service.ProvideCanvasRoutingService(userRepository, groupRepository, userSubscriptionRepository, accountRepository, billingService, mediaCatalogService, generationJobRepository, canvasResourceRouteRepository)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, apiKeyCache, configConfig, billingCacheService, canvasRoutingService)
	...
	signupBonusIPLimiter := repository.NewSignupBonusIPLimiter(redisClient)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, affiliateService, serviceUserPlatformQuotaRepository, signupBonusIPLimiter)
	...
	canvasHandler := handler.NewCanvasHandler(apiKeyService, userService, canvasRoutingService, channelService)
	...
	ideaMessageRepository := repository.NewIdeaMessageRepository(client)
	ideaMessageService := service.NewIdeaMessageService(ideaMessageRepository, userRepository)
	ideaMessageHandler := handler.NewIdeaMessageHandler(ideaMessageService)
	...
	xaioAuthClient := repository.NewXAIOAuthClient()
	xaioAuthService := service.ProvideXAIOAuthService(proxyRepository, xaioAuthClient)
	accountTestService := service.NewAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, xaioAuthService)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	accountHandler := admin.NewAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator)
	adminAnnouncementHandler := admin.NewAnnouncementHandler(announcementService)
	adminIdeaMessageHandler := admin.NewIdeaMessageHandler(ideaMessageService)
	...
	xaioAuthHandler := admin.NewXAIOAuthHandler(xaioAuthService, adminService)
	...
	retailGrokKeyRepository := repository.NewRetailGrokKeyRepository(db)
	...
	leonardoImageGenerationClientFactory := service.ProvideLeonardoImageAccountAdapterFactory(httpUpstream, configConfig)
	leonardoImageUploadCache := repository.NewLeonardoImageUploadCache(redisClient)
	leonardoImageUploadService := service.NewLeonardoImageUploadService(leonardoImageUploadCache)
	leonardoImageCreateOrchestrator := service.ProvideLeonardoImageCreateOrchestrator(leonardoImageQuoteGuard, leonardoImageCreateFunds, accountRepository, leonardoImageGenerationClientFactory, generationJobRepository, leonardoImageUploadService)
	...
	mediaFundsRepository := repository.NewMediaFundsRepository(client, db)
	mediaFundsService := service.NewMediaFundsService(mediaFundsRepository)
	mediaJobAttemptRepository := repository.NewMediaJobAttemptRepository(db)
	mediaUsageAuditRepository := repository.NewMediaUsageAuditRepository(client)
	mediaUsageAuditService := service.NewMediaUsageAuditService(mediaUsageAuditRepository)
	defaultMediaOpenAIExecutor := service.NewDefaultMediaOpenAIExecutor(openAIGatewayService, accountRepository)
	mediaOpenAIAdapter := service.NewMediaOpenAIAdapter(defaultMediaOpenAIExecutor)
	defaultMediaLeonardoExecutor := service.NewDefaultMediaLeonardoExecutor(leonardoMediaCreateService, leonardoMediaGetService)
	mediaLeonardoAdapter := service.NewMediaLeonardoAdapter(defaultMediaLeonardoExecutor)
	mediaOrchestrator := service.ProvideMediaOrchestrator(mediaCatalogService, generationJobRepository, accountRepository, mediaFundsService, mediaJobAttemptRepository, mediaUsageAuditService, client, concurrencyService, mediaOpenAIAdapter, mediaLeonardoAdapter)
	mediaHandler := handler.NewMediaHandler(mediaCatalogService, mediaOrchestrator, contentModerationService, billingCacheService, concurrencyService, openAIGatewayService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, canvasHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, ideaMessageHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, retailGrokGatewayHandler, playgroundHandler, leonardoMediaHandler, leonardoWebhookHandler, mediaHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService)
	apiKeyAuthMiddleware := middleware.NewAPIKeyAuthMiddleware(apiKeyService, subscriptionService, configConfig)
	retailGrokKeyAuthMiddleware := middleware.NewRetailGrokKeyAuthMiddleware(retailGrokService)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, retailGrokKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, redisClient)
